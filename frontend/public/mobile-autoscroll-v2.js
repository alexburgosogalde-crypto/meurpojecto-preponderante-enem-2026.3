...
(function(){
  ...
  // ---------- 1) Auto-scroll (only for chat pages) ----------
  // Skip auto-scroll on pages that aren't chat-based (e.g. pagamento, donaspainel).
  // We detect chat pages by the presence of .css-uiqgnq (chat list container).
  var lastHeight = 0;
  var scrollIfNeeded = function() {
    try {
      if (!document.querySelector('.css-uiqgnq')) return; // not a chat page
